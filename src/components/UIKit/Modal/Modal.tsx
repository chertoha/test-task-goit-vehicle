...
const Modal: FC<IModalProps> = ({ children, close, isOpen }) => {
  ...
  return (
    ...
    createPortal(
      ...
      >
        <div className="absolute left-1/2 top-10 flex -translate-x-1/2 xl:w-[982px] rounded-[20px]  max-h-[calc(100%-80px)] bg-white p-10 pr-4">
          <button
            className="absolute right-10 top-10 transition-all duration-300 ease-in-out hover:scale-110 bg-white rounded-md"
            onClick={close}
          >
            <CloseIcon
              width={32}
